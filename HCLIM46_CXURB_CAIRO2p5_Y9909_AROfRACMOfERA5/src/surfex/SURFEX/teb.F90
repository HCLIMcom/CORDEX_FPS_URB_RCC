...
!   ##########################################################################
    SUBROUTINE TEB  (TOP, T, BOP, B, TIR, DMT, HIMPLICIT_WIND, PTSUN,                   &
                     ...
                     PEMIT_LW_RD, PT_RAD_IND, PHU_BLD, PTIME, PE_SHADING, YSPP )
!   ##########################################################################
!
!!****  *TEB*  
!!
...
REAL, DIMENSION(SIZE(PTA)) :: ZIMB_WL      ! wall residual energy imbalance 
                                             ! for verification [W m-2]
REAL, DIMENSION(SIZE(PTA)) :: ZTS_TOWN     ! averaged surface temperature over town [K]
!
...
!
 CALL URBAN_DRAG(TOP, T, B, HIMPLICIT_WIND, PTSTEP, PT_CANYON, PQ_CANYON, &
                 ...
                 PDUWDU_RD, PDUWDU_RF, PUSTAR_TWN, ZAC_WIN, ZTS_TOWN, YSPP )
! * ts town
DMT%XTS_TOWN(:) = ZTS_TOWN(:)
!
