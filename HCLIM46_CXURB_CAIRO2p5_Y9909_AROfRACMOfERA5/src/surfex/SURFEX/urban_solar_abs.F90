...
!     #########
    SUBROUTINE URBAN_SOLAR_ABS(TOP, T, B, DMT, PDIR_SW, PSCA_SW, PZENITH, PAZIM,   &
                               ...
                               PE_SHADING, OSHAD_DAY, OSHADE, PDIR_SW_RD,           &
                               OALB_ONLY                                            )
!   ##########################################################################
!
!!****  *URBAN_SOLAR_ABS*  
...
!
REAL, DIMENSION(:), INTENT(OUT)    :: PDIR_SW_RD       ! direct solar radiation received by snow-free roads
!
...
REAL, DIMENSION(SIZE(PDIR_SW)) :: ZDIR_SW_WL_A        ! direct radiation reaching wall A,
...
!               -------------------------------------------------------
!
  PDIR_SW_RD (JJ) = (  ZDIR_SW(JJ) * 2. * ZTHETA0(JJ) / XPI                 &
...
!              ----------------------------------------
!
  ZDIR_SW_WL(JJ) = (ZDIR_SW(JJ) - (PDIR_SW_RD(JJ)*ZRD(JJ)+ZDIR_SW_GD(JJ)*ZGD(JJ))) &
...
!
!
 CALL SOLAR_REFLECTIONS(PDIR_SW_RD, ZDIR_SW_WL, ZDIR_SW_GD, ZABS_DIR_SW_RD, &
