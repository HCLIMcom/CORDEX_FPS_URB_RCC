...
USE MODD_SLT_SURF
!
USE MODD_SFX_SPP, ONLY : SPP_ALL_t
!
USE MODE_DSLT_SURF
USE MODE_THERMOS
USE MODE_SBLS
...
!
TYPE(SPP_ALL_t), INTENT(INOUT) :: YSPP
!
...
REAL, DIMENSION(KI)  :: ZDIR_SW_RD    ! direct SW received by roads
...
DO JP = 1,TOP%NTEB_PATCH
  ...
  CALL TEB_GARDEN(DTCO, G, TOP, NT%AL(JP), BOP, NB%AL(JP), TPN, TIR, TD%NDMT%AL(JP), GDM, GRM, JP, &
                  ...
                  ZREF_SW_GRND, ZREF_SW_FAC, ZHU_BLD, PTIME, ZPROD_BLD, ZDIR_SW_RD, YSPP )
  ...
  IF (TD%O%N2M >0 .AND. TD%DUT%LUTCI) THEN
    ...
    CALL ADD_PATCH_CONTRIB(JP,ZAVG_DIR_SW_ROAD ,ZDIR_SW_RD   )
...
IF (TD%DUT%LUTCI .AND. TD%O%N2M >0) THEN
 ...
 DO JJ=1,KI
   IF (ZAVG_DIR_SW(JJ).GT.0.) THEN
     TD%DUT%XUTCI_OUTAGG(JJ) = TD%DUT%XUTCI_OUTSUN  (JJ)*(   ZAVG_DIR_SW_ROAD(JJ)/ZAVG_DIR_SW(JJ)) &
                             + TD%DUT%XUTCI_OUTSHADE(JJ)*(1.-ZAVG_DIR_SW_ROAD(JJ)/ZAVG_DIR_SW(JJ))
   ELSE
     TD%DUT%XUTCI_OUTAGG(JJ) = TD%DUT%XUTCI_OUTSHADE(JJ)
   ENDIF
 ENDDO
 !
