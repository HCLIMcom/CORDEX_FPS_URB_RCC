...
!
  IF (.NOT. TOP%LCANOPY) THEN
    ...
    ! Diagnostic of canyon temperature
    ...
    PT_CAN(JJ) =  ( ZRD(JJ)      * T%XT_ROAD  (JJ,1) * PAC_RD (JJ)    * PDF_RD (JJ)        &
                   ...
                   + PHSN_RD(JJ) * PDN_RD(JJ) * ZRD(JJ)                 / PRHOA(JJ) / XCPD  ) &
                                            / ZINTER  
!
    PT_CAN(JJ) = PT_CAN(JJ) + ( ZGD(JJ) * PTSRAD_GD(JJ) * PAC_GD(JJ) ) / ZINTER
    !

    ! Diagnostic of canyon temperature for road only
    DMT%XT_RD_CAN(JJ) =  (  T%XT_ROAD  (JJ,1) * PAC_RD (JJ) * (PDF_RD(JJ)*ZRD(JJ)+ZGD(JJ))           &
                          + T%XT_WALL_A(JJ,1) * PAC_WL (JJ) * (1.-B%XGR(JJ)) * PWL_O_GRND(JJ) * 0.5  &
                          + T%XT_WALL_B(JJ,1) * PAC_WL (JJ) * (1.-B%XGR(JJ)) * PWL_O_GRND(JJ) * 0.5  &
                          + B%XT_WIN1    (JJ) * PAC_WL (JJ) *     B%XGR(JJ)  * PWL_O_GRND(JJ)        &
                          + PTA          (JJ) * PAC_TOP(JJ)                                          &
                          + PH_TRAFFIC   (JJ) / (1.-T%XBLD (JJ))               / PRHOA(JJ) / XCPD    &
                          + PHSN_RD(JJ) * PDN_RD(JJ) * ZRD(JJ)                 / PRHOA(JJ) / XCPD  ) &
                          / (PAC_RD(JJ) * (PDF_RD(JJ)*ZRD(JJ)+ZGD(JJ)) + PAC_WL(JJ) * PWL_O_GRND(JJ) + PAC_TOP(JJ))
    !
    ! Diagnostic of canyon temperature for garden only
    IF (T%XGARDEN(JJ) /= 0.) THEN
      DMT%XT_GD_CAN(JJ) =  (  PTSRAD_GD(JJ)     * PAC_GD(JJ)  * (ZGD(JJ)+ZRD(JJ))                      &
                          + T%XT_WALL_A(JJ,1) * PAC_WL (JJ) * (1.-B%XGR(JJ)) * PWL_O_GRND(JJ) * 0.5  &
                          + T%XT_WALL_B(JJ,1) * PAC_WL (JJ) * (1.-B%XGR(JJ)) * PWL_O_GRND(JJ) * 0.5  &
                          + B%XT_WIN1    (JJ) * PAC_WL (JJ) *     B%XGR(JJ)  * PWL_O_GRND(JJ)        &
                          + PTA          (JJ) * PAC_TOP(JJ)                                        ) &
                          / (PAC_GD(JJ)*(ZGD(JJ)+ZRD(JJ)) + PAC_WL(JJ)*PWL_O_GRND(JJ) + PAC_TOP(JJ))
    END IF
    !
    IF (TOP%CBEM=="BEM") THEN
      PT_CAN(JJ) = PT_CAN(JJ) + (DMT%XH_WASTE(JJ) * B%XF_WASTE_CAN(JJ) / (1-T%XBLD(JJ)) / PRHOA(JJ) / XCPD) / ZINTER
      DMT%XT_RD_CAN(JJ) = DMT%XT_RD_CAN(JJ) + (DMT%XH_WASTE(JJ) * B%XF_WASTE_CAN(JJ) / (1-T%XBLD(JJ)) / PRHOA(JJ) / XCPD) / ZINTER
      IF (T%XGARDEN(JJ) /= 0.) THEN
        DMT%XT_GD_CAN(JJ) = DMT%XT_GD_CAN(JJ) + (DMT%XH_WASTE(JJ) * B%XF_WASTE_CAN(JJ) / (1-T%XBLD(JJ)) / PRHOA(JJ) / XCPD) / ZINTER
      ENDIF
    ENDIF
    !
    ! Diagnostic of canyon temperature
    DMT%XT_CAN(JJ) = PT_CAN(JJ)
!
!-------------------------------------------------------------------------------
!
...
!
    ...
    !
    ! Diagnostic of canyon humidity for road only
    DMT%XQ_RD_CAN(JJ) = (  PQSAT_RD   (JJ) * PAC_RD_WAT(JJ) * (PDF_RD    (JJ) * ZRD(JJ) * PDELT_RD(JJ) + ZGD(JJ))   &
                         + PQA        (JJ) * PAC_TOP(JJ)                                                            &
                         + PLE_TRAFFIC(JJ) / (1.-T%XBLD(JJ)) / PRHOA(JJ) / XLVTT                                    &
                         + PLESN_RD   (JJ) * PDN_RD(JJ)      / PRHOA(JJ) / XLVTT * ZRD(JJ)                        ) &
                         / (PAC_RD_WAT(JJ) * (PDF_RD(JJ)*PDELT_RD(JJ)*ZRD(JJ)+ZGD(JJ)) + PAC_TOP(JJ))
    !
    ! Diagnostic of canyon humidity for garden only
    IF (T%XGARDEN(JJ) /= 0.) THEN
        DMT%XQ_GD_CAN(JJ) = (  PQSAT_GD   (JJ) * PAC_AGG_GD(JJ) * PHU_AGG_GD(JJ) * (ZGD(JJ) +ZRD(JJ))                   &
                         + PQA        (JJ) * PAC_TOP(JJ)                                                          ) &
                         / (PAC_AGG_GD(JJ) * PHU_AGG_GD(JJ) * (ZGD(JJ)+ZRD(JJ)) + PAC_TOP(JJ))
    ENDIF
    !
    IF (TOP%CBEM=="BEM") THEN
      PQ_CAN(JJ) = PQ_CAN(JJ) + (DMT%XLE_WASTE(JJ) * B%XF_WASTE_CAN(JJ) / (1-T%XBLD(JJ)) / PRHOA(JJ) / XLVTT) / ZINTER
      DMT%XQ_RD_CAN(JJ) = DMT%XQ_RD_CAN(JJ) + (DMT%XLE_WASTE(JJ) * B%XF_WASTE_CAN(JJ) / (1-T%XBLD(JJ)) / PRHOA(JJ) / XLVTT) / ZINTER
      DMT%XQ_GD_CAN(JJ) = DMT%XQ_GD_CAN(JJ) + (DMT%XLE_WASTE(JJ) * B%XF_WASTE_CAN(JJ) / (1-T%XBLD(JJ)) / PRHOA(JJ) / XLVTT) / ZINTER
    ENDIF
    !
  ENDIF
!
...
!
!
! Diagnostic of canyon specif. humidity
DMT%XQ_CAN(:) = PQ_CAN(:)
!
! Diagnostic of canyon relative humidity
DMT%XHU_CAN(:) =  PQ_CAN(:) / QSAT (PT_CAN(:), PPS(:))
!
ZQSAT_CAN(:) = QSAT (DMT%XT_RD_CAN(:), PPS(:))
WHERE (DMT%XQ_RD_CAN(:) > ZQSAT_CAN(:))
   DMT%XQ_RD_CAN(:) = ZQSAT_CAN(:)
END WHERE
!
ZQSAT_CAN(:) = QSAT (DMT%XT_GD_CAN(:), PPS(:))
WHERE (DMT%XQ_GD_CAN(:) > ZQSAT_CAN(:))
   DMT%XQ_GD_CAN(:) = ZQSAT_CAN(:)
END WHERE
!
