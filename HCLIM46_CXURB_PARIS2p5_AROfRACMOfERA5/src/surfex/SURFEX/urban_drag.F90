...
!     #########
    SUBROUTINE URBAN_DRAG(TOP, T, B, HIMPLICIT_WIND, PTSTEP, PT_CANYON, PQ_CANYON, &
                          ...
                          PDUWDU_ROOF, PUSTAR_TOWN, PAC_WIN, PTS_TOWN, YSPP )
!   ##########################################################################
!
!!****  *URBAN_DRAG*  
!!
...
!
REAL, DIMENSION(:), INTENT(OUT)   :: PTS_TOWN       ! town averaged surface temperature
!
...
!              ----------------------------------
!
  PTS_TOWN(JJ) = T%XBLD(JJ) * PTS_ROOF(JJ) + (1.-T%XBLD(JJ)) * PT_CANYON(JJ)
!
...
IF (.NOT. TOP%LCANOPY) THEN
  CALL URBAN_EXCH_COEF(TOP%CZ0H, ZZ0_O_Z0H, PTS_TOWN, ZQ_TOWN, PEXNS, PEXNA, PTA, PQA,     &
                       PZREF+ T%XBLD_HEIGHT/3., PUREF+T%XBLD_HEIGHT/3., PVMOD, T%XZ0_TOWN, &
                       PRI, PCD, PCDN, ZAC, ZRA, ZCH, YSPP                        )
ENDIF
!
!-------------------------------------------------------------------------------
!
...
DO JLOOP=1,3
 !
  ...
  CALL URBAN_EXCH_COEF(TOP%CZ0H, ZZ0_O_Z0H, PTS_ROAD, PQ_LOWCAN, PEXNS, PEXNA,  &
                       ...
                       PAC_ROAD, ZRA_ROAD, ZCH_ROAD, YSPP )
  !
  DO JJ=1,SIZE(PTA)
