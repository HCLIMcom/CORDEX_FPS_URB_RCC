...
BINDIR=${BINDIR-$HM_DATA/../HCLIM46_CXURB_PARIS2p5_AROfRACMOfERA5/bin}                 # Binary directory
...
[ "$FP_PRECISION" = "dual" ] && BINDIR=$HM_DATA/../HCLIM46_CXURB_PARIS2p5_AROfRACMOfERA5/bin/R64
...
# **** Nesting ****
HOST_MODEL="racmo"                      # Host model (ifs|hir|ald|ala|aro|racmo)
                                        # ifs  : ecmwf data
                                        # hir  : hirlam data
...
NBDMAX=15                               # Number of parallel interpolation tasks
MULTITASK=no                            # Submit jobs through the multi-task script
                                        #  yes|no, or list among: lbc,listen,obsmon,fldextr,femars,pertdia
BDLIB=RACMO                             # Boundary experiment, set:
                                        # ECMWF to use MARS data
                                        # RCRa  to use RCRa data from ECFS
                                        # Other HARMONIE/HIRLAM experiment

BDDIR=$HM_DATA/${BDLIB}/archive/@YYYY@/@MM@             # Boundary file directory,
                                                        # For more information, read in scr/Boundary_strategy.pl
# BERT: RACMO data are stored in ECFS in ec:/rus/HXEUR12/eR2v3-v578ec-Y2023-LU2015-MAC2SP-fERA5/RACMO_HarmLBC
EXT_ACCESS="tar xvf"                    # untar the files
EXT_BDDIR="$SCRATCH/HARMONIE/boundaries/RACMO/HXEUR12/eR2v3-v578ec-Y2023-LU2015-MAC2SP-fERA5/RACMO_HarmLBC" #  "/ec/res4/scratch/rus/XRAC/HXEUR12/eR2v3-v578ec-Y2023-LU2015-MAC2SP-fERA5/RACMO_HarmLBC"
EXT_BDFILE="RC-eR2v3-v578ec-Y2023-LU2015-MAC2SP-fERA5_@YYYY@@MM@_d@X@.tar"
...
BDSTRATEGY=analysis_only        # Which boundary strategy to follow
...
# **** Climate files ****
CREATE_CLIMATE=${CREATE_CLIMATE-no}    # Run climate generation (yes|no)
CLIMDIR=$HM_DATA/../HCLIM46_CXURB_PARIS2p5_AROfRACMOfERA5/climate/$DOMAIN        # Climate files directory
...
NTIMEFMT=11                             # Time format in FA output file names
                                        #   0: +{HHHH} or +{step} (if TFLAG=min)
                                        #   1: +{HHHH}:{mm}
                                        #  10: +{HHHH}h
                                        #  11: +{HHHH}h{mm}m
NVGRIB=0                                # Level of GRIB packing for atmosphere FA files
...
RCATVARS="sftnf:sftof:sfturf:sftlaf,orog"      # Copy nc files of these variables to RCATDIR, colon separated list
...
MAIL_ON_ABORT=ulft@knmi.nl              # when a task aborts
