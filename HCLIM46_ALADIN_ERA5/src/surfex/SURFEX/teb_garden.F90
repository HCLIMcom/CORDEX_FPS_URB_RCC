...
!
REAL, DIMENSION(:),   INTENT(OUT)     :: PDIR_SW_RD       ! Direct solar radiation received by roads
!
...
!
  CALL GARDEN(DTCO, G, T, TOP, TIR, GDM%DTV, GDM%GB, GDDK, GDDEK, GDDMK,                &
              ...
              ZAC_AGG_GD, ZHU_AGG_GD, DMT%XIRRIG_GARDEN, YSPP )
...
ENDIF
!
! * Surface temperature for green areas
DMT%XTS_GARDEN(:) = ZTSRAD_GD(:)
!
...
IF (TOP%LGREENROOF) THEN
  ...
  CALL GREENROOF(DTCO, G, T, TOP, TIR, GRM%DTV, GRM%GB, GRDK, GRDEK,                 &
                 ...
                 DMT%XG_GREENROOF_ROOF, DMT%XIRRIG_GREENROOF, YSPP )
  !
  PAC_GR_WAT(:) = PAC_GR(:)
...
!
  CALL TEB  (TOP, T, BOP, B, TIR, DMT, HIMPLICIT_WIND,                             &
             ...
             PT_RAD_IND, PHU_BLD, PTIME, ZE_SHADING, YSPP )
!
!-------------------------------------------------------------------------------
!
